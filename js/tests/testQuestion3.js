...
QUnit.test( "test content of memory address 30", function( assert ) {
  for(var i = 0; i < submissions.length; i++) {
    ...
    assert.equal( result[30], 15, submissions[i].studentID + ": the 30th memory address should contain 15" );
  }
});
...
QUnit.test( "test content of memory address 30", function( assert ) {
  for(var i = 0; i < submissions.length; i++) {
    ...
    var testProgram = testInputs + userProgram;
    var result = LoadProgram(testProgram);
    assert.equal( result[30], 0, submissions[i].studentID + ": the 30th memory address should contain 0" );
  }
});
...
QUnit.test( "test content of memory address 30", function( assert ) {
  for(var i = 0; i < submissions.length; i++) {
    ...
    var testProgram = testInputs + userProgram;
    var result = LoadProgram(testProgram);
    assert.equal( result[30], 15, submissions[i].studentID + ": the 30th memory address should contain 5" );
  }
});
